...
export async function POST(request: Request) {
  ...
  if (
    ...
    body.updates.length > stocks.length * (LAST_ROUND + 1)
  ) {
    return Response.json(
      { error: "수정할 주가를 다시 확인해 주세요." },
      { status: 400 },
    );
  }
  ...
  if (
    updates.some(
      (item) =>
        !stocks.some((stock) => stock.ticker === item.ticker) ||
        !Number.isInteger(item.round) ||
        item.round < firstEditableRound ||
    ...
    return Response.json(
      { error: "진행되지 않은 라운드의 올바른 주가만 수정할 수 있습니다." },
      { status: 400 },
    );
  }
  await db.batch([
    ...updates.map((item) =>
      db
        ...
        .bind(item.price, item.ticker, item.round),
    ),
    db.prepare(
    ...
    adminAuditStatement(
      "price_update",
      `미공개 주가 ${updates.length}개를 수정했습니다.`,
      { updates },
    ),
  ]);
