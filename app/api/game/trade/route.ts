...
export async function POST(request: Request) {
  ...
  if (!session || session.role !== "team") return Response.json({ error: "조 계정으로 로그인해 주세요." }, { status: 403 });
  const body = await request.json() as { ticker?: string; action?: "buy" | "sell"; quantity?: number };
  const ticker = String(body.ticker ?? "");
  const action = body.action;
  const quantity = Number(body.quantity);
  if (!stocks.some((stock) => stock.ticker === ticker) || !["buy", "sell"].includes(String(action)) || !Number.isInteger(quantity) || quantity < 1 || quantity > 1000000) {
    return Response.json({ error: "주문 내용을 다시 확인해 주세요." }, { status: 400 });
  }
  ...
  if (action === "buy" && (!team || team.cash < total)) return Response.json({ error: "보유 BE Coin이 부족합니다." }, { status: 400 });
  if (action === "sell" && shares < quantity) return Response.json({ error: "보유한 수량보다 많이 팔 수 없습니다." }, { status: 400 });
  const nextCash = (team?.cash ?? 0) + (action === "buy" ? -total : total);
  const nextShares = shares + (action === "buy" ? quantity : -quantity);
  await db.batch([
    db.prepare("UPDATE teams SET cash = ? WHERE team_id = ?").bind(nextCash, session.teamId),
    db.prepare(`INSERT INTO holdings (team_id, ticker, shares) VALUES (?, ?, ?)
      ON CONFLICT(team_id, ticker) DO UPDATE SET shares = excluded.shares`).bind(session.teamId, ticker, nextShares),
    db.prepare("INSERT INTO trades (team_id, ticker, action, quantity, price, round) VALUES (?, ?, ?, ?, ?, ?)").bind(session.teamId, ticker, action, quantity, price, game.round),
  ]);
