...
import { getStockPrice } from "../../game-data";
...
export async function GET() {
  ...
  const db = getGameDb();
  const [gameResult, teamResult, holdingResult, tradeResult] = await Promise.all([
  ...
  const teamViews = (teamResult.results ?? []).map((team) => {
    ...
    const stockValue = teamHoldings.reduce((sum, item) => sum + item.shares * (getStockPrice(item.ticker, game.round) ?? 0), 0);
    return {
      teamId: team.team_id,
