...
export async function GET() {
  ...
  return session
    ? Response.json({ session: publicGameSession(session) })
...
export async function POST(request: Request) {
  ...
  const configuredTeam =
    Number.isInteger(teamId) && teamId >= 1 && teamId <= 30
      ? await getGameDb()
  ...
  if (/^\d{1,2}$/.test(id) && configuredTeam && password === teamPassword) {
    ...
    const session: GameSession = {
      role: "team",
      teamId,
      sessionVersion: sessionRow.session_version,
    };
    await setSession(session, request);
    return Response.json({ session: publicGameSession(session) });
  }
